...
function kiosk_log_event(string $message, array $context = []): void {
    $parts = [];
    foreach ($context as $key => $value) {
        $parts[] = trim((string)$key) . '=' . trim((string)$value);
    }

    $line = sprintf('[%s] [network-dashboard] %s', date('Y-m-d H:i:s'), trim($message));
    if ($parts !== []) {
        $line .= ' ' . implode(' ', $parts);
    }

    @error_log($line . PHP_EOL, 3, '/home/pi/kiosk-runtime.log');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        if ($ssid === '' || $psk === '') {
            ...
            kiosk_log_event('Wi-Fi update aangevraagd via dashboard.', ['ssid' => $ssid, 'hidden' => $hidden, 'country' => $country]);
            // Refresh status
            $current_ssid = trim(shell_exec("iwgetid -r 2>/dev/null"));
            $current_ip   = trim(shell_exec("hostname -I 2>/dev/null"));
        }
        ...
        $status_msg = "<pre style='background:#eef;padding:10px;border-radius:6px;white-space:pre-wrap'>".esc_html($out)."</pre>";
        kiosk_log_event('Wi-Fi reconfigure aangevraagd via dashboard.');
        $current_ssid = trim(shell_exec("iwgetid -r 2>/dev/null"));
        $current_ip   = trim(shell_exec("hostname -I 2>/dev/null"));
    }
}
